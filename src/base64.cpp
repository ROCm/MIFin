...
static unsigned int pos_of_char(const unsigned char chr) {
  ...
  if (chr >= 'A' && chr <= 'Z') {
    ...
    retval = chr - '0' + ('Z' - 'A') + ('z' - 'a') + 2;
  } else if (chr == '+' || chr == '-') {
    retval = 62;
  } // Be liberal with input and accept both url ('-') and non-url ('+') base 64
    // characters (
  else if (chr == '/' || chr == '_') {
    retval = 63;
  } // Ditto for '/' and '_'
...
template <typename String> static std::string encode(String s, bool url) {
  return base64_encode(static_cast<const unsigned char *>(s.data()), s.length(),
                       url);
}
