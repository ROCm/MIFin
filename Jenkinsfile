                ...
                stage('Hip Tidy') {
                    ...
                    environment{
                        cmd = "rm -rf build; \
                                mkdir build; \
                                cd build; \
                                CXX=/opt/rocm/llvm/bin/clang++ cmake -DCMAKE_PREFIX_PATH="/root/dMIOpen/cget;/opt/rocm" -DBUILD_DEV=On ..; \
                ...
                stage('Build Fin') {
                    ...
                    environment{
                        cmd = "rm -rf build; \
                                mkdir build; \
                                cd build; \
                                CXX=/opt/rocm/llvm/bin/clang++ cmake -DBUILD_DEV=On -DCMAKE_PREFIX_PATH="/root/dMIOpen/cget;/opt/rocm" ..; \
                                make -j\$(nproc) all;"
                    }
                    steps{
                        buildJob('clang++', '-DCMAKE_BUILD_TYPE=release', image, "", cmd)
